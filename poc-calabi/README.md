...
6. prove that the Copy Fail AF_ALG/authencesn path, BPF entry points, packet sockets, user namespaces, io_uring, and Dirty Frag / Fragnesia xfrm/RxRPC/AF_ALG entry points return permission denied;
...
- the Dirty Frag / Fragnesia probe prints `BLOCKED` for `NETLINK_XFRM`, `AF_RXRPC`, and Fragnesia `AF_ALG`;
