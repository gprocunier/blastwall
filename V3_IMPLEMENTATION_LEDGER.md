...
- Blastwall v3 is publishable as a reference exemplar; named governance owners
  and sign-off are adopter responsibilities before local operation.
- Stable-v3 service-owned custody health is live-green in the Calabi
  demonstration environment as of KRA health `4872`, policy pipeline `4922`,
  runtime workflow `4968`, and inventory audit `4989`.
- Fleet-scale readiness should wait for the broader mixed-state scale gate.
...
- Broader production operation remains an adopter decision backed by local
  ownership, review, retention, and scale evidence.
...
- open issues: standalone runtime launches must provide `BLASTWALL_CURRENT_POLICY_SHA256`; additional destructive rehearsals remain optional pending external reviewer request
...
- open issues: adopter governance owner assignment and fleet-scale evidence remain future adoption work
...
- open issues: governance owners remain pending for adopters; fleet-scale evidence remains future work
...
### Phase 18
- phase: 18 stable-v3 service-custody remediation and live capture
- files changed: `playbooks/attestation-vault-health.yml`, `playbooks/preflight.yml`, `playbooks/sign-attestation.yml`, `playbooks/negative-gate-attestation-artifacts.yml`, `aap/configure-controller.yml`, `tests/policy_static.py`, `docs/blastwall-v3/*`, `release/STABLE_V3_DECISION.md`, `V3_IMPLEMENTATION_LEDGER.md`
- tests added: static guards for omitting false `shared` parameters on non-shared custody paths, reference-exemplar release language, operational guidance links, and runtime workflow candidate preflight targeting
- tests run: `python3 tests/policy_static.py`; `python3 -m pytest -q tests/test_blastwall_attestation_vault.py tests/test_blastwall_attestation_sign.py`; live AAP project sync `4871`; KRA health `4872`; shared-custody rejection `4876`; candidate preflight `4918`; policy pipeline `4922`; runtime workflow `4968`; inventory audit `4989`
- open issues: governance owners and sign-off remain adopter work; fleet-scale evidence remains future work
- security invariants checked: stable-v3 rejects shared vault custody; non-shared custody uses the service path without passing false `shared`; runtime preflight targets the candidate host group explicitly; intentional broken fixtures fail closed
