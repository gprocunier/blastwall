...
Latest Controller-visible recapture: project sync `4221` to
...
| Digest mismatch | `FAIL_ATTESTATION_INTEGRITY` | final live job `4233` failed closed as `FAIL_ATTESTATION_INTEGRITY`; historical job `3457` failed closed before normalization | artifact `4222`, mutation `4226`, inventory `4230`, preflight `4233` | `stale-blastwall-01.workshop.lan` | source tests reject breakglass | `4237`, sync `4241` | none |
...
| Revoked marker | `FAIL_REVOKED_ATTESTATION` | final live job `4255` failed closed as `FAIL_REVOKED_ATTESTATION`; historical job `3601` failed closed before normalization | artifact `4244`, mutation `4248`, inventory `4252`, preflight `4255` | `stale-blastwall-01.workshop.lan` | source tests reject breakglass | `4259`, sync `4263`; final restore `4266`, sync `4270` | none |
...
index, and envelope now maps to `FAIL_ATTESTATION_INTEGRITY`. Both normalized
paths were re-captured live after Controller project sync `4221`.
